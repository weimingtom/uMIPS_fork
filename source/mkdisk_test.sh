...
sudo mkdir /mnt/1
dd if=/dev/zero of=disktemp bs=1M count=128
sudo mkfs.vfat disktemp
sudo losetup /dev/loop0 disktemp
...
dd if=disktemp of=disk bs=8192 seek=1 count=16384 conv=notrunc 2>/dev/null
